...
function test_input($data) {
 ...
 $bFname = test_input($_POST["BorrowerFirstName"]);
 $bLname = test_input($_POST["BorrowerLastName"]);
global $returnQ;
	...
	if($bFname=="" && $bLname ==""){
		...
		if($bLname ==""){
			$returnQ = "SELECT BL.Book_id, BL.Branch_id, BL.Card_no, Date_out, Due_date, Date_in, BL.Loan_id, B.Fname, B.Lname
					...
					WHERE (B.Fname LIKE '%$bFname%') AND BL.Card_no = B.Card_no
		...
		if($bFname==""){
			$returnQ = "SELECT BL.Book_id, BL.Branch_id, BL.Card_no, Date_out, Due_date, Date_in, BL.Loan_id, B.Fname, B.Lname
					FROM book_loans BL, borrower B
					WHERE (B.Lname LIKE '%$bLname%') AND BL.Card_no = B.Card_no
					ORDER BY BL.Card_no, BL.Loan_id";
		}
		if( ($bFname !="") && ($bLname !="")){
			$returnQ = "SELECT BL.Book_id, BL.Branch_id, BL.Card_no, Date_out, Due_date, Date_in, BL.Loan_id, B.Fname, B.Lname
					FROM book_loans BL, borrower B
					WHERE (B.Fname LIKE '%$bFname%') AND (B.Lname LIKE '%$bLname%') AND BL.Card_no = B.Card_no
					ORDER BY BL.Card_no, BL.Loan_id";
		}
